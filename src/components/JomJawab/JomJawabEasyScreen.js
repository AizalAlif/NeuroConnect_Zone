...
export default function JomJawabEasyScreen({ navigation }) {
    ...
    const fetchQuestions = async () => {
      try {
        ...
        if (snapshot.exists()) {
          ...
          const fetchedQuestions = Object.values(data); // Convert to array
...
useEffect(() => {
  const loadVictorySound = async () => {
    try {
      ...
      console.log('Error loading victory sound:', error);
    }
  };
  loadVictorySound();
...
const handleNextQuestion = async () => {
  playButtonSound(); // Play sound on next question
  if (currentQuestionIndex < questions.length - 1) {
    ...
    if (victorySound) await victorySound.playAsync(); // Play victory sound
    Alert.alert(
      'Tahniah!',
  ...
  const fetchLeaderboard = async () => {
    try {
      const leaderboardQuery = query(ref(database, 'leaderboard/'), orderByChild('score'));
  ...
  const renderMedia = () => {
    if (currentQuestionIndex === 0) {
      return (
        <Image
          source={require('../../../assets/laut.jpeg')}
          style={styles.media}
        />
      );
      ...
      return (
        <Image
          source={require('../../../assets/awan.jpg')}
          style={styles.media}
        />
      );
              ...
              <View style={styles.modalContent}>
                ...
                <Button
                  ...
                  onPress={() => {
                    ...
                    navigation.navigate('LeaderboardJomJawabEasy');
                  }}
                />
              </View>
            </View>
