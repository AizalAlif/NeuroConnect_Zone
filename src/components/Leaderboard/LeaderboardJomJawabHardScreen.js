...
export default function LeaderboardJomJawabHard({ navigation }) {
  ...
  useEffect(() => {
    const fetchScores = async () => {
      try {
        const snapshot = await get(ref(database, 'leaderboard_hard/'));
        if (snapshot.exists()) {
  ...
  const resetLeaderboard = async () => {
    try {
      await remove(ref(database, 'leaderboard_hard/'));
      setScores([]);
