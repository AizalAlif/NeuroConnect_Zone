...
export default function LeaderboardJomJawabMedium({ navigation }) {
  ...
  useEffect(() => {
    const fetchScores = async () => {
      try {
        const snapshot = await get(ref(database, 'leaderboard_medium/'));
        if (snapshot.exists()) {
  ...
  const resetLeaderboard = async () => {
    try {
      await remove(ref(database, 'leaderboard_medium/'));
      setScores([]);
  ...
  const renderTopRanks = (item, index) => {
    ...
    return (
      <View style={podiumStyles} key={item.userId}>
        ...
        <Text style={styles.topRankScore}>{item.score} pts</Text>
      </View>
    );
  };
    ...
    <ImageBackground source={require('../../../assets/background15.jpeg')} style={styles.backgroundImage}>
      <View style={styles.container}>
        <View style={styles.headerContainer}>
          ...
          <Text style={styles.subtitle}>Jom Jawab Medium</Text>
        </View>
