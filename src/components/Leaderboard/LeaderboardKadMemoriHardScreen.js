...
export default function LeaderboardKadMemoriHard({ navigation }) {
  ...
  useEffect(() => {
    const fetchScores = async () => {
      try {
        const snapshot = await get(ref(database, 'kadMemoriHardleaderboard/'));
        if (snapshot.exists()) {
  ...
  const resetLeaderboard = async () => {
    try {
      await remove(ref(database, 'kadMemoriHardleaderboard/'));
      setScores([]);
    ...
    <ImageBackground source={require('../../../assets/background15.jpeg')} style={styles.backgroundImage}>
      <View style={styles.container}>
        <View style={styles.headerContainer}>
          ...
          <Text style={styles.subtitle}>Kad Memori Hard</Text>
        </View>
