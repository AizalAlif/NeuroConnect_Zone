...
export default function LeaderboardKadMemoriMedium({ navigation }) {
  ...
  useEffect(() => {
    const fetchScores = async () => {
      try {
        const snapshot = await get(ref(database, 'kadMemoriMediumLeaderboard/')); // Corrected path
        if (snapshot.exists()) {
          ...
          // Sort scores by ascending order of moves
  ...
  const resetLeaderboard = async () => {
    try {
      await remove(ref(database, 'kadMemoriMediumLeaderboard/')); // Corrected path
      setScores([]);
    ...
    <ImageBackground source={require('../../../assets/background15.jpeg')} style={styles.backgroundImage}>
      <View style={styles.container}>
        <View style={styles.headerContainer}>
          ...
          <Text style={styles.subtitle}>Kad Memori Medium</Text>
        </View>
